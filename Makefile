...
test:
	...
	$(EXEC) create -k $(TEST)/TACK_Key1.pem -c $(CERT1) > $(TEST)/TACK1.pem	
	$(EXEC) c -k $(TEST)/TACK_Key1.pem -p asdf -c $(CERT2) -o $(TEST)/TACK2.pem		
	$(EXEC) c -k $(TEST)/TACK_Key1.pem -p asdf -s v1_key -c $(CERT1) -o $(TEST)/TACK3.pem			
	$(EXEC) c -k $(TEST)/TACK_Key1.pem -p asdf -s v1_cert -d 30d -c $(CERT1) -o $(TEST)/TACK4.pem			
	$(EXEC) c -k $(TEST)/TACK_Key1.pem -p asdf -s v1_key -d 1d12h5m -e 2030-06-06Z -c $(CERT1) -o $(TEST)/TACK5.pem
	$(EXEC) c -k $(TEST)/TACK_Key1.pem -p asdf -s v1_key -g 1 -c $(CERT1) -o $(TEST)/TACK6.pem
